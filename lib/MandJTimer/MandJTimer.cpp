...
MandJTimer::MandJTimer(void)
{

}

int8_t MandJTimer::every(unsigned long period, void (*callback)(), int repeatCount)
{
	...
	_events[i].lastEventTime = myTimer.readTimer(); //millis();
	_events[i].count = 0;
	return i;
}

int8_t MandJTimer::every(unsigned long period, void (*callback)())
{
	return every(period, callback, -1); // - means forever
}

int8_t MandJTimer::after(unsigned long period, void (*callback)())
{
	return every(period, callback, 1);
}

int8_t MandJTimer::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue, int repeatCount)
{
	int8_t i = findFreeEventIndex();
	...
	_events[i].lastEventTime = myTimer.readTimer();
	_events[i].count = 0;
	return i;
}

int8_t MandJTimer::oscillate(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t MandJTimer::pulse(uint8_t pin, unsigned long period, uint8_t startingValue)
{
 ...
 */
int8_t MandJTimer::pulseImmediate(uint8_t pin, unsigned long period, uint8_t pulseValue)
{
...
void MandJTimer::stop(int8_t id)
{
	if (id >= 0 && id < MAX_NUMBER_OF_EVENTS) {
...
void MandJTimer::update(void)
{
	unsigned long now = myTimer.readTimer(); //millis();
	update(now);
}

void MandJTimer::update(unsigned long now)
{
...
int8_t MandJTimer::findFreeEventIndex(void)
{
	for (int8_t i = 0; i < MAX_NUMBER_OF_EVENTS; i++)
	{
		if (_events[i].eventType == EVENT_NONE)
...
unsigned long MandJTimer::readTimer() {
	return myTimer.readTimer();
}

void MandJTimer::startTimer()
{
	myTimer.startTimer();
}
