...
void setup() {
  ...
  timerLCDbacklight = allarm.mjTimer.every(settings.lcdBacklightTime,
                                           timerDoLCDbacklight);
  //
  timerReadGSMSlave = allarm.mjTimer.every(1, readGSMSlave);
...
void keypadEvent(KeypadEvent eKey) {
  ...
  switch (keypad.getState()) {
    case PRESSED:
      ...
      allarm.lcd.backlight();
      allarm.mjTimer.stop(timerLCDbacklight);
      timerLCDbacklight = allarm.mjTimer.every(settings.lcdBacklightTime,
                                               timerDoLCDbacklight);
...
void doPrintRitAttivazione() {
  //standby();
  allarm.lcd.setCursor(0, 2);
  allarm.lcd.print(F("Rit. "));
  allarm.lcd.print(++conta);
...
MandJBeep::MandJBeep(MJLcd& _lcd)
  : lcd(_lcd) {
...
void MandJBeep::codiceErrato(char adm = 0) {
  ...
  if (adm == 0) {
    lcd.mjPrint(3,0,TXT_INVALID_PIN);
  } else {
    password.set(settings.alarmPassword1);
    lcd.mjPrint(0,0,TXT_INVALID_ADMIN_PIN);
  }
  lcd.mjPrint(5,2,TXT_RIPROVA_PIN);
  delay(1000);
  allarm.standby();
}
...
void MandJBeep::primaDiAttivare() {
  ...
  if (this->checkSensori()) {
    ...
    this->attiva();
    evRitardoAttivazione = this->mjTimer.every(1, doPrintRitAttivazione, settings.tempoRitardo);
    evAfterRitardoTrigger = this->mjTimer.after(settings.tempoRitardo, doAfterRitActivate);
    this->standby();
  }
}
...
void MandJBeep::disattiva() {
  ...
  if (this->ritardoAttivato) {
    ...
    this->mjTimer.stop(evRitardoAttivazione);
    this->mjTimer.stop(evAfterRitardoTrigger);
    this->ritardoAttivato = true;
  }
  ...
  lcd.mjPrint(0,0,TXT_SISTEMA_DISATTIVO);
...
void MandJBeep::alarmTriggered() {
  ...
  lcd.mjPrint(5,2,TXT_INTRUSIONE);
  lcd.mjPrint(0, 3,"");
  ...
  mjTimer.after(settings.tempoSirena, doAfterTimerT);
}
...
void MandJBeep::alarmTriggeredRitardato(uint8_t sensId) {
  ...
  if (allarm.ritardoAttivato == false)  // controlla se altro evento già avviato
  {
    ...
    evRitardoAttivazione = mjTimer.every(1, doPrintRitAttivazione, settings.tempoRitardo);
    evAfterRitardoTrigger = mjTimer.after(settings.tempoRitardo, doAfterRitardoTrigged);
  }
}
